...
Dv = 0;
Dw = 0.2;
%Dw=0.8, A=4,B=1-->We get the ( 2nd row 4th fig(r)).
%SPOD+defect/shuffling
%Dw = 0.2(best)/0.23,A=9.5,B=2.2-->travelling spod, travelling chimera
%Dw=0.25,A=5, B=0.8-->Gs+defect(1st row 4th fig(r))
%Also A=4,B=0.9, D=0.25(good)
%Dw=0.2, A=5.5,B=2-->APS, APS+defect(Also,Dw=0.13,A10,B2.9)
A = 5.5;
B = 2;
sv = (Dv/2)*dt/(2*(dx^2));
sw = (Dw/2)*dt/(2*(dx^2));
...
for ii=1:nt
      ...
      reac_v0 = B+(V0.^2).*W0-(1+A)*V0;
      reac_w0 = A*V0-(V0.^2).*W0 ;
          
     reac_v1 = B+(V1.^2).*W1-(1+A)*V1;
     reac_w1 = A*V1-(V1.^2).*W1 ;
